...
from .utils import open_file_wrapped
...
class AtomicFactGenerator(object):
  ...
  def __init__(
    ...
    if 'llama' in model_name.lower():
        self.get_model_response.stop_tokens = ["<|eot_id|>"]
        if 'instruct' not in model_name.lower():
            self.get_model_response.stop_tokens.append("Please breakdown the following sentence")
  ...
  def get_init_atomic_facts_from_sentence(self, sentences, cost_estimate=None):
    ...
    if cost_estimate:
      ...
      for prompt in prompts:
        ...
        system_message = ""
        response, prompt_tok_cnt, response_tok_cnt =self.get_model_response.get_response(system_message, prompt_to_send, cost_estimate_only=cost_estimate)

        sentences_from_output = text_to_sentences(response)

        if not sentences_from_output:  # account for markdown lists
          sentences_from_output = text_to_sentences(response, separator='* ')
